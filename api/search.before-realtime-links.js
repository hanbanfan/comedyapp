...
export default async function handler(req, res) {
  ...
  if (!apiKey) {
    return res.status(500).json({
      ...
      checkedAt,
      warning: "SERPAPI_KEY is missing. Accurate live search is disabled until the key is loaded.",
      results: [],
    });
  }
  ...
  try {
    ...
    return res.status(200).json({
      ...
      checkedAt,
      accuracyNote: "Live discovery results are source-backed, not JokeFlow-verified. Open the linked source to confirm dates, prices, lineups, and submission rules.",
      results,
    });
  } catch (error) {
    return res.status(500).json({
